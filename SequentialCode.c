...
{
    FILE *fichier = NULL;
    // Ouverture du bon fichier texte selon la difficulté choisis
    if (dl == 0.004)
    {
        fichier = fopen("TimeSequentialFast.txt", "a");
    }

    if (dl == 0.002)
    {
        fichier = fopen("TimeSequentialMedium.txt", "a");
    }

    if (dl == 0.001)
    {
        fichier = fopen("TimeSequentialNormal.txt", "a");
    }
    if (fichier == NULL)
    {
    ...
    // Fonctions permettant de mesurer le temps
    clock_t start, end;
    ...
    printf("Le temps d'execution de la fonction est %f secondes.\n", cpu_time_used);
